...
<head>
    ...
    <!-- Style CSS -->
    <link rel="stylesheet" href="<?php echo base_url(); ?>/assets/frontend/css/style.css?version=1.5">    <!-- Responsive  CSS -->
...
<body class="hold-transition <?php echo (base_url() == str_replace('/index.php/','',$uri)) ? 'home' : ''; ?> " >
...
<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
  ...
  <symbol id="grid" viewBox="0 0 16 16">
	...
	<header class="sticky-top" id="header" style="display: <?php echo !empty($from_cron) ? 'none' : ''; ?>">
		<p class="marquee">
	   <span>
		   Sign up to receive a 30 Day FREE Trial!
	   </span>
	   <span>
		   Sign up to receive a 30 Day FREE Trial!
	   </span>
	   ...
	   <span>
		...
		<div class="bg-orange">
		<div class="container-fluid px-xl-5 d-flex justify-content-center">
			<ul class="navbar-nav ms-sm-auto align-items-center d-flex flex-row text-white gap-4">
				<?php if (session()->get('vr_sess_logged_in') != TRUE) : ?>
					<li class="nav-item d-none d-sm-block"><a class="nav-link text-white fs-7" href="<?php echo base_url('providerauth/login'); ?>"><svg class="bi mx-auto" width="20" height="20"><use xlink:href="#people-circle"></use></svg> Login</a></li>
					<!--<li class="nav-item ms-3"><a class="nav-link text-white" href="<?php echo base_url('providerauth/register'); ?>">Provider Signup</a></li>-->					
				<?php else : ?>					
					<li class="nav-item"><span class="nav-link fs-7"><svg xmlns="http://www.w3.org/2000/svg" class="ioniconswhites" viewBox="0 0 512 512"><path d="M258.9 48C141.92 46.42 46.42 141.92 48 258.9c1.56 112.19 92.91 203.54 205.1 205.1 117 1.6 212.48-93.9 210.88-210.88C462.44 140.91 371.09 49.56 258.9 48zm126.42 327.25a4 4 0 01-6.14-.32 124.27 124.27 0 00-32.35-29.59C321.37 329 289.11 320 256 320s-65.37 9-90.83 25.34a124.24 124.24 0 00-32.35 29.58 4 4 0 01-6.14.32A175.32 175.32 0 0180 259c-1.63-97.31 78.22-178.76 175.57-179S432 158.81 432 256a175.32 175.32 0 01-46.68 119.25z"/><path d="M256 144c-19.72 0-37.55 7.39-50.22 20.82s-19 32-17.57 51.93C191.11 256 221.52 288 256 288s64.83-32 67.79-71.24c1.48-19.74-4.8-38.14-17.68-51.82C293.39 151.44 275.59 144 256 144z"/></svg><?php echo '<span>Welcome </span>'.getFirstName(session()->get('vr_sess_user_id')); ?></span></li>
					<li class="nav-item ms-0">
						<ol class="d-flex gap-4">
						<li class="nav-item"><a class="nav-link fs-7" href="<?php echo base_url('providerauth/dashboard'); ?>">Dashboard</a></li>
						<li class="nav-item"><a class="nav-link fs-7" href="<?php echo base_url('providerauth/logout'); ?>">Logout</a></li>
...
<?php
 $currentsegment = ($uri->getTotalSegments() >= (env('urlsegment')-1) && !empty($uri->getSegment(env('urlsegment')-1))) ? $uri->getSegment(env('urlsegment')-1) : ''; ?>
		...
		<div class="row w-100 m-auto align-items-center justify-content-between">
		  ...
		  <div class="collapse navbar-collapse col-sm-9" id="navbarCollapse">
			...
			<?php if (session()->get('vr_sess_logged_in') != TRUE) : ?>
			  ...
			  <?php else : ?>
				<li class="nav-item d-none d-sm-block">			  
				<a class="btn bg-orange" href="<?php echo base_url('providerauth/dashboard'); ?>">Dashboard</a>
				</li>
			<?php endif; ?>
			  ...
			  if(getPlanId(session()->get('vr_sess_user_id')) != 3){ ?>
			  <li class="nav-item">
				<a class="btn yellowbtn " href="<?php echo base_url('providerauth/plan'); ?>">UPGRADE MY PLAN</a>
			  </li>
			  <?php } } ?>
    ...
    <footer style="display: <?php echo !empty($from_cron) ? 'none' : ''; ?>">
		<div class="container py-5 py-sm-5 text-center">
			<img title="CodeIgniter Logo" alt="Visit CodeIgniter.com official website!" src="<?= base_url('assets/img/flogo.png') ?>">
...
#gdpr-cookie-message input[type="checkbox"] {
	...
	<?php } ?>
	<script src="<?php echo base_url(); ?>/assets/frontend/js/provider.js?version=1.3"></script>
