...
//echo "<pre>";print_r($data);exit;
    ...
    public function register()
    {
        ...
        $data['status'] = 1;
        $data['token'] = generate_unique_id();
        $data['role'] = $data['role'] ? $data['role'] : '1';
	...
	public function get_user_photos($id,$plan_id=''){
		if($plan_id == 1 || $plan_id == 2){
			$sql = "SELECT * FROM user_images WHERE user_images.user_id = ? order by user_images.order asc LIMIT 1";
		}else{
			$sql = "SELECT * FROM user_images WHERE user_images.user_id = ? order by user_images.order asc";
		}		
        $query = $this->db->query($sql, array($id));
        return $query->getResultArray();
	}
