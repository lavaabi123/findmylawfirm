...
class Providers extends BaseController
{
	...
	public function view_profile($category_name = null, $userId = null){
		...
		if((!empty($paramUserId) && is_numeric($paramUserId))){
			...
			if($userId == null && empty($paramUserId) && !empty(session()->get('vr_sess_user_id'))){
		...
		}elseif($userId == null && empty($paramUserId) && !empty(session()->get('vr_sess_user_id'))){
			$userId = session()->get('vr_sess_user_id');
		}
		...
		$data['featured'] = $this->CityModel->get_featured($data['user_detail']->location_id,$data['user_detail']->permalink,$radius=1);
	...
	public function providers_list($location = null, $category = null)
    {	
		...
		//echo $location_id;
		//echo $category;
		//$data['categories'] = $this->CityModel->get_categories($location_id,$category,$radius);
		/*if( ($location_id == 'all' && $category == 'all') ){
			$data['categories'] = $this->CityModel->get_categories3(null,$radius);
		}else*/
