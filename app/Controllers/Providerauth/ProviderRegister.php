...
class ProviderRegister extends ProviderauthController
{
            ...
            if ($user) {
                ...
                if ($userModel->is_logged_in()) {
                    if(session()->get('vr_sess_user_role') > 1){
                        return redirect()->to(base_url('providerauth/plan'));
                    }/*else{
    ...
    {   
		...
		$data['user_id'] = session()->get('vr_sess_user_id');
    ...
    {  
		if ($this->session->get('vr_sess_logged_in') != TRUE) {
            ...
            return redirect()->to(base_url('/providerauth/register'));
        }
		$this->UsersModel = new UsersModel();
		...
		if($data['user_detail']->plan_id == '1' && $this->request->getVar('id') == '1'){
			...
			if($this->request->getVar('id') >= '2'){
				//$user = $this->UsersModel->update_user_plan(session()->get('vr_sess_user_id'),$this->request->getVar('id'));
				...
				<a href="'.base_url('/provider/'.$busin_name).'">
					<div class="provider-Details mb-4">
						...
						<div class="pro-content">
							<p class="text-grey mb-0 fw-bold">'.$provider['name'].'</p>
							<p class="text-orange mb-0 fw-bold">'.$provider['business_name'].'</p>
							<h6 class="text-grey">'.$provider['city'].', '.$provider['state_code'].' '.$provider['zipcode'].'</h6>
						</div>
					</div>
				</a>
			</div>';
