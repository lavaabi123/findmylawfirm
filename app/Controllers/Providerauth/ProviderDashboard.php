...
class ProviderDashboard extends ProviderauthController
{
    ...
    public function index()
    {
		...
		$data['view_count'] = $this->UsersModel->db->query('SELECT count(*) as view_count FROM `website_stats` where view_count = 1 AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"')->getRow()->view_count;
		$data['call_count'] = $this->UsersModel->db->query('SELECT count(*) as call_count FROM `website_stats` where call_count = 1 AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"')->getRow()->call_count;
		$data['form_count'] = $this->UsersModel->db->query('SELECT count(*) as form_count FROM `website_stats` where form_count = 1 AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"')->getRow()->form_count;
		$data['direction_count'] = $this->UsersModel->db->query('SELECT count(*) as direction_count FROM `website_stats` where direction_count = 1 AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"')->getRow()->direction_count;
		...
		$data['impression_count'] = $this->UsersModel->db->query('SELECT count(*) as impression_count FROM `website_stats` where impression_count = 1 AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"')->getRow()->impression_count;
		
		
		$query2 =  $this->userModel->db->query('SELECT count(*) as user_count, zipcodes.city, zipcodes.zipcode, states.code as state_code FROM `website_stats` w LEFT JOIN zipcodes ON w.customer_zipcode = zipcodes.zipcode LEFT JOIN states ON zipcodes.state_id = states.id where w.customer_zipcode != ""  AND w.user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'" AND city != "" GROUP BY w.customer_zipcode order by user_count desc  limit 10')->getResult();
		$query1 =  $this->UsersModel->db->query('SELECT * FROM `website_stats` where customer_zipcode != ""  AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'" GROUP BY created_at order by created_at desc LIMIT 20')->getResult();
		$query3 =  $this->UsersModel->db->query('SELECT * FROM `website_stats` where call_count = 1  AND user_id = '.session()->get('vr_sess_user_id').' AND created_at <= "'.$endDate.'" AND created_at >= "'.$startDate.'"  order by created_at desc LIMIT 20')->getResult();
		//print_r($query1);exit;	
	...
	public function edit_profile(){
		...
		$data['rate_details'] = $this->UsersModel->get_rate_details(session()->get('vr_sess_user_id'));
		$data['hours_of_operation'] = $this->UsersModel->get_hours_of_operation(session()->get('vr_sess_user_id'));
	...
	public function get_categories_skills(){
		...
		if($this->request->getPost('edit_user_id') === null) {
			$userId = session()->get('vr_sess_user_id');
		}else{
	...
	public function get_category_offering(){
		$this->OfferingModel = new OfferingModel();
		$category_offering = $this->OfferingModel->get_offering_category_based(1);
		$html = '';		
		//check if category id is already selected one
		$this->UsersModel = new UsersModel();
	...
	public function photos_post(){
		if($this->request->getVar('check') == '1'){
			$this->UsersModel = new UsersModel();
			$user_detail = $this->UsersModel->get_user(session()->get('vr_sess_user_id'));
			$photos = $this->UsersModel->get_user_photos(session()->get('vr_sess_user_id'));
			$response = '1';
			...
			$this->UsersModel = new UsersModel();
			if(!empty(session()->get('vr_sess_user_id'))){
				$user = $this->UsersModel->update_user_profile_photo($this->request->getVar('image'),session()->get('vr_sess_user_id'));
			}
			echo '1';
		}else{
			$this->UsersModel = new UsersModel();
			$user = $this->UsersModel->insert_user_photos($this->request->getVar('image'),session()->get('vr_sess_user_id'),$this->request->getVar('image_tag'));
			if ($user) {
				$photos = $this->UsersModel->get_user_photos(session()->get('vr_sess_user_id'));
				$html = '';
				if(!empty($photos)){
	...
	public function photosedit_post(){
			...
			$user = $this->UsersModel->update_user_photos($this->request->getVar('p_id'),session()->get('vr_sess_user_id'),$this->request->getVar('image_tag'),$image);
			if ($user) {
				$photos = $this->UsersModel->get_user_photos(session()->get('vr_sess_user_id'));
				$html = '';
				if(!empty($photos)){
...
/*
				...
				$data_insert = [
					'user_id'                         => session()->get('vr_sess_user_id'),
				...
				$user = $this->UsersModel->update_user_plan(session()->get('vr_sess_user_id'),$updateUser);
				if(!empty($this->request->getVar('type')) && $this->request->getVar('type') == 'trial'){
	...
	private function createStripeCustomer($token)
    {
        ...
        $customer = Customer::create([
            'email' => session()->get('vr_sess_user_email'),
			'source' => $token,
	...
	public function messages(){
        ...
        $pagination = $this->paginate($this->UsersModel->get_paginated_provider_messages_count(session()->get('vr_sess_user_id')));
        $data['provider_messages'] =   $this->UsersModel->get_paginated_provider_messages($pagination['per_page'], $pagination['offset'],session()->get('vr_sess_user_id'));
	...
	function update_hiring_status(){
		if(!empty(session()->get('vr_sess_user_id'))){
			$this->UsersModel = new UsersModel();
			$user_detail = $this->UsersModel->update_hiring_status($_POST['hiring_status'],session()->get('vr_sess_user_id'));
			echo 'success';			
		}else{
			echo 'failed';
		}
